...
void services::showDescription(QListBoxItem* input)
{
	...
	else if( input->text().contains("mysql") )
		text = i18n("MySQL is a fast, stable and true multi-user, multi-threaded SQL database server.");
	else if( input->text().contains("nvidia") )
		text = i18n("These binary drivers provide optimized hardware acceleration of  OpenGL applications via a direct-rendering X Server.");
	else if( input->text() == "powersaved" )
		text = i18n("Support for ACPI, APM and CPU frequency scaling.");
	...
	else if( input->text() == "sqlrelay" )
		text = i18n("SQL Relay is a persistent database connection pooling, proxying and load balancing system.");
	...
	else if( input->text() == "timidity" )
		text = i18n("TiMidity++ is a very high quality software-only MIDI sequencer.");
